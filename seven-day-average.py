...
def comparative_averages(new_cases, states):
    for state in states:
        ...
        try:
            percent_change = ((this_week_average - last_week_average) / last_week_average) * 100
        except ZeroDivisionError:
            percent_change = 0.0

        print(f"State: {state}")
        print(f"This week's 7-day average: {this_week_average:.2f}")
        print(f"Last week's 7-day average: {last_week_average:.2f}")
